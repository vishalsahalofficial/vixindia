...
public class SignupService {
    ...
    public ResponseEntity<?> sendOTPForSignup(MobileVerificationRequest mobileVerificationRequest) {
        ...
        try {
            ...
            if (mobileVerificationRequest.getMobileNumber() != null && mobileVerificationRequest.getMobileNumber() != 0) {
                Boolean isValid = validator.validateMobile(mobileVerificationRequest.getMobileNumber().toString());
                if (!isValid) {
                    throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Please enter Valid Mobile Number", new ArrayList<>());
                }
                User userDetails = userDao.findByMobileNumber(mobileVerificationRequest.getMobileNumber().toString());
                if (userDetails != null) {
                    throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "User Already Exists with this mobile number", new ArrayList<>());
                }
                verifyDetails = signupDao.getMobileVeificationOTPCount(mobileVerificationRequest.getMobileNumber());
                ...
                if (verifyDetails != null &&
                    ...
                    throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Maximum OTP limit exceeded, please try after 5 minutes.", new ArrayList<>());
    ...
    public ResponseEntity<?> validateMobile(MobileValidationRequest mobileValidationRequest) {
        ...
        if (mobileValidationRequest.getOtp() != null && !mobileValidationRequest.getOtp().isEmpty()) {
            Boolean isValid = validator.validateMobile(mobileValidationRequest.getMobileNumber().toString());
            if (!isValid) {
                throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Please enter Valid Mobile Number", new ArrayList<>());
            }
            signupDao.isMobileNumberAlreadyVerified(mobileValidationRequest.getMobileNumber());
            ResponseEntity<?> errorList = validateMobileOtp(mobileValidationRequest.getMobileNumber().toString(), mobileValidationRequest.getOtp());
        ...
        try {
            if (recentOtpDetails != null) {
                if (recentOtpDetails.get("mobile_otp_try_count") != null
                    ...
                    throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Maximum otp try limit reached. Please try after 5 minutes", new ArrayList<>());
                ...
                if (recentOtpDetails.get("mobile_otp") != null
                    ...
                    throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Invalid OTP, Please check and try again.", new ArrayList<>());

                }
            } else {
                throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "OTP Expired.", new ArrayList<>());
            }
        } catch (GlobalException e) {
    ...
    public ResponseEntity<?> addCustomer(SignUpRequest signUpRequest) {
        ...
        if (validator.validateSignup(signUpRequest) == null) {
            Long userId = signupDao.insertProfile(signUpRequest.getName(), signUpRequest.getMobileNumber());
