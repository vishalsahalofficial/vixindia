...
public class UserDaoImpl implements UserDao {
        ...
        try {
            ...
            return jdbcTemplate.queryForObject(sql, namedParameters, new RowMapper<User>() {
                ...
                public User mapRow(ResultSet rs, int rowNum) throws SQLException {
                    ...
                    user.setOtpTryCount(rs.getInt("otp_try_count"));
                    return user;
                }
            });
    ...
    public Boolean isNeedToSendOTP(String mobileNumber) {
        String SQL = "SELECT otp_try_count, timestampdiff(MINUTE,mobile_otp_update_time,current_timestamp()) AS otp_life_time FROM user_profile " +
        ...
        try {
            ...
            if (tryCount >= 3 && OTPLifeTime < 5) {
                APP_LOG.info("OTP IS NOT SENT BECAUSE OTP TRY COUNT IS " + tryCount + "  AND OTP LIFE TIME IS " + OTPLifeTime + " MIN.");
            ...
            throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Mobile number is not registered", new ArrayList<>());
        } catch (Exception e) {
            throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Something Went Wrong", new ArrayList<>());
        }
    }

    @Override
    public void updateOtpTryCountAndVerification(String mobileNumber, Integer tryCount, Integer isMobileVerified) {
        ...
        try {
            ...
            throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Something Went Wrong While updateOtpTryCountAndVerification", new ArrayList<>());
        }
    }
    ...
    private void resetTryCount(String mobileNumber) {
        ...
        try {
            SqlParameterSource parameterSource = new MapSqlParameterSource().addValue("mobileNumber", mobileNumber);
            jdbcTemplate.update(SQL, parameterSource);
        } catch (Exception e) {
            throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Something Went Wrong While Resetting OTP try count", new ArrayList<>());
        }
    }
