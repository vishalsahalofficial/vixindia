...
public class Validator {
    ...
    public Boolean validateEmail(String email) {
        Pattern mobilePattern = Pattern.compile("^[\\w!#$%&'*+/=?`{|}~^-]+(?:\\.[\\w!#$%&'*+/=?`{|}~^-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,6}$");
    ...
    public Integer validateSignup(SignUpRequest request) {
        ...
        if (request.getName().trim().isEmpty()) {
            throw new GlobalException(HttpStatus.BAD_REQUEST.value(), "Name can't be empty", new ArrayList<>());
        }
